...
Seeder memakai gambar placeholder dari **Lorem Picsum** (`picsum.photos`, layanan foto bebas pakai untuk pengembangan) berdasarkan seed nama artikel, supaya tiap artikel punya cover + galeri yang konsisten sejak awal. Ganti `cover_image`/`gallery` di database dengan foto asli Kabupaten Malang (dokumentasi Pemkab/Diskominfo) begitu tersedia — kolom `cover_image` cukup diisi URL atau path storage.
