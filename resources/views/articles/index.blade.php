...
@section('content')
    <section class="bg-ijotebu contour-bg py-16">
        <div class="max-w-6xl mx-auto px-5">
            <span class="font-mono text-xs uppercase tracking-widest text-emas">{{ $category->slug === 'kecamatan' ? '33 Wilayah Administratif' : 'Profil Daerah' }}</span>
    ...
    <section class="max-w-6xl mx-auto px-5 py-14">
        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach($articles as $article)
                <a href="{{ route('article.show', [$category->slug, $article]) }}" class="group block bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition">
